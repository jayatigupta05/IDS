...
#     def detect_multiport_scan(self, packet):
#         """Detects if one source hits multiple destination ports quickly."""
#         if IP in packet:
#             scan_tracker = {}

#         for (src_ip, _, _, dport), _ in self.tracker.connections.items():
#             if src_ip not in scan_tracker:
#                 scan_tracker[src_ip] = set()
#             scan_tracker[src_ip].add(dport)

#             if len(scan_tracker[src_ip]) > 20:
#                 print(f"[ALERT] Port scan from {src_ip} ({len(scan_tracker[src_ip])} ports)"
#                 f"({len(self.tracker.connections[src_ip]['ports'])} ports)")
    ...
    def detect_batch_port_scans(self):
        ...
        for (src, _, _, dport), _ in self.tracker.connections.items():
            if src not in scan_tracker and dport != None:
...
class Sniffer:
    ...
    def sniff_packets(self, interface="eth0"):
        sniff(iface="Wi-Fi", filter="tcp or udp or icmp", prn=self.process_packet, store=0)
